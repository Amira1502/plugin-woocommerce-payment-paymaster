...
function woocommerce_paymaster()
{
    ...
    class WC_PAYMASTER extends WC_Payment_Gateway
    {
        public function __construct()
        {
            ...
            $this->liveurl = 'https://psp.paymaster.tn/payment/test/d92c3015-5212-4f0e-b8ea-3498a53e1ae3?amount=1';
            // Load the settings
        ...
        public function admin_options()
        {
            ...
            print '<p>'._e('Merchant PAYMASTER.', 'woocommerce').'</p>';
        ...
        public function init_form_fields()
        {
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'label'       => 'Enable paymaster Gateway',
                    'type'        => 'checkbox',
                    'description' => '',
                    'default'     => 'no'
                ),
        ...
        public function generate_form($order_id)
        {
            ...
            $args = array(
                ...
                'LMI_PAYMENT_DESC' => 'order №' . $order_id,
            ...
            $pos = 0;
            //We receive products in the form
            ...
            $args["LMI_SHOPPINGCART.ITEM[{$pos}].NAME"] = 'order №' . $order_id;
            ...
            return
                ...
                implode("\n", $args_array) .
                '<input type="submit" class="button alt" id="submit_paymaster_payment_form" value="' . __('Pay', 'woocommerce') . '" /> <a class="button cancel" href="' . $order->get_cancel_order_url() . '">' . __('Refuse payment & return to cart', 'woocommerce') . '</a>' . "\n" .
                '</form>';
        }
         ...
         * attention not to "fence the garden" with the transfer and verification of an additional parameter
         * SIGN type, as in previous versions of the module, I made it simpler, see the comments inside the function
         * @param $posted
         * @return string
         */
        public function get_hash($posted)
        {
            // We get the seller's ID not from the POST request, but from the module (we exclude, thereby its substitution)
            $LMI_MERCHANT_ID = $this->paymaster_merchant;        
           //We got the order number, we really need it, see below what we will do with it    
           ...
           //And here are the "chips" that prevent attackers from replacing HASH :)
            //Namely, we take LMI_PAYMENT_AMOUNT and LMI_CURRENCY not from the post request, but from the module
            $order = new WC_Order($LMI_PAYMENT_NO);
            $LMI_PAYMENT_AMOUNT = $this->getOrderTotal($order);
            //Now we get the currency of the order, what was in the order
            $LMI_CURRENCY = $order->currency;
        ...
        /**
         * We return the order amount in the format 0.00
         **/
        ...
        private function add_transaction_info($post)
        {
            ...
            $order = new WC_Order($orderId);
            $message = 'The transaction was carried out on the website of the Paymaster payment system. Transaction number: ' .
                $post['LMI_SYS_PAYMENT_ID'] . '. ' .
                'payment date: ' . $post['LMI_SYS_PAYMENT_DATE'] . '. ' .
                'Currency and payment amount: ' . $post['LMI_PAID_AMOUNT'] . ' ' . $post['LMI_PAID_CURRENCY'] . '. ' .
                'Payment method selected by the user: ' . $post['LMI_PAYMENT_METHOD'] . '. ' .
                'Payer ID in the payment system: ' . $post['LMI_PAYER_IDENTIFIER'] . '. ' .
                'IP payer s address: ' . $post['LMI_PAYER_IP_ADDRESS'] . '.';
